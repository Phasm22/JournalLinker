...
2. Resolve active journal context (`resolve_current_journal_context`) — finds the current note date from the input text header, `--active-date`/`--active-file` flags, or the newest modified `YYYY-MM-DD.md` in the journal dir
...
6. Output to stdout; update learning state and nav links in journal files as side effects
